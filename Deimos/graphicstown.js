...
window.onload = function() {
    ...
    uiMode.innerHTML += "<option>Overview</option>";
    ...
    inputNum.setAttribute('type', 'num');
    inputNum.setAttribute('value', '11');
    controls.appendChild(inputNum);

    var inputButton = document.createElement("button");
    inputButton.innerHTML = "Input Waypoint";
    inputButton.onclick = function()
    {
        console.log(inputNum.getAttribute('value'));
    };
    ...
    var drivePos = [100,15,0];
    var driveTheta = -Math.PI/2;
    var driveXTheta = 0;

    //Overview parameters
    var overviewPos = [0,1000 ,0];
    var overviewX = 0;
    var overviewZ = 0;
    ...
    function draw() {
        ...
        var projM = twgl.m4.perspective(fov, 1, 0.1, 10000);
        ...
        if (uiMode.value == "ArcBall") {
            ...
            twgl.m4.setTranslation(viewM, [0, -25, -250], viewM);
            ...
            if (keysdown[87] || keysdown[40]) {
            overviewX += .04;
            }
            else if (keysdown[83] || keysdown[38]) {
            overviewX -= .04;
            }
            if (overviewZ > 0)
                overviewZ -= .02;
            ...
            overviewPos = [overviewPos[0] + overviewX, overviewPos[1], overviewPos[2]+overviewZ];
            cameraM = twgl.m4.lookAt(overviewPos, [overviewPos[0],0,overviewPos[2]], [1,0,0]);
            //twgl.m4.multiply(cameraM, twgl.m4.rotationY(driveTheta), cameraM);
            //twgl.m4.setTranslation(cameraM, overviewPos, cameraM);
            viewM = twgl.m4.inverse(cameraM);
        }

        // get lighting information
        var tod = Number(sliders.TimeOfDay.value);
        var sunAngle = Math.PI * (tod-6)/12 + Math.PI * 1.5;
        var sunDirection = [Math.cos(sunAngle),0,Math.sin(sunAngle)];
